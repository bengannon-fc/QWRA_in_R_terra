####################################################################################################
#### Calculate conditional and expected Net Value Change
...
###########################################START MESSAGE############################################
cat('Calculate conditional and expected Net Value Change\n',sep='')
...
for(i in 1:length(HVRAs)){ # Loop through HVRAs
	...
	for(j in 1:length(sHVRAs)){
		...
		for(k in 1:nrow(ct_s)){
			...
			if(exists('vmask')){
				cNVC_sc.l[[k]] <- cNVC_sc*ct_s$RIPP[k]*HVRA_bin*rmask
			}else{
				cNVC_sc.l[[k]] <- cNVC_sc*ct_s$RIPP[k]*HVRA_bin
			}
			...
			if(exists('tlr')){
				...
				tlr <- ifel(!is.na(HVRA_bin),ct_s$RIPP[k],NA)
			}
			
		}
		
		#-> Create sub-HVRA cNVC raster
		cNVC_s.l[[j]] <- sum(rast(cNVC_sc.l),na.rm=T)
		if(sHVRAs[j] != 'None'){
			writeRaster(cNVC_s.l[[j]],paste0(OUTp,'/',HVRAs[i],'_',sHVRAs[j],'_cNVC.tif'),
			            overwrite=T)
		}
	...
	cNVC.l[[i]] <- sum(rast(cNVC_s.l),na.rm=T)
	writeRaster(cNVC.l[[i]],paste0(OUTp,'/',HVRAs[i],'_cNVC.tif'),overwrite=T)
	
	#-> Create HVRA eNVC raster
	writeRaster(cNVC.l[[i]]*bp,paste0(OUTp,'/',HVRAs[i],'_eNVC.tif'),overwrite=T)
